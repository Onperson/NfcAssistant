...
public class AccessConditionTool extends BasicActivity {
     ...
     * using {@link NfcAssistantApplication#acBytesToACMatrix(byte[])},
     * {@link #acBitsToACRowNr(byte[], boolean)},
     * {@link #getResourceForDataBlocksByRowNr(int)} and
     * {@link #getResourceForSectorTrailersByRowNr(int)}.
     * @param view The View object that triggered the method
     * (in this case the decode button).
     * @see NfcAssistantApplication#acBytesToACMatrix(byte[])
    ...
    public void onDecode(View view) {
        ...
        byte[][] acMatrix = NfcAssistantApplication.acBytesToACMatrix(
                NfcAssistantApplication.hexStringToByteArray(ac));
        boolean error = false;
        if (acMatrix != null) {
     ...
     * {@link NfcAssistantApplication#acMatrixToACBytes(byte[][])} and display them.
     * @param view The View object that triggered the method
     * (in this case the encode button).
     * @see NfcAssistantApplication#acMatrixToACBytes(byte[][])
     * @see #mACMatrix
     * @see #acRowNrToACBits(int, boolean)
     */
    public void onEncode(View view) {
        mAC.setText(NfcAssistantApplication.byte2HexString(NfcAssistantApplication.acMatrixToACBytes(mACMatrix)));
    }
    ...
    public void onCopyToClipboard(View view) {
        NfcAssistantApplication.copyToClipboard(mAC.getText().toString(), this, true);
    }
