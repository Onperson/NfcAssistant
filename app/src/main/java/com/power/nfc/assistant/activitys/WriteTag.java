...
public class WriteTag extends BasicActivity {
    ...
    public final static String EXTRA_DUMP =
            "com.power.nfc.assistant.Activity.DUMP";
    ...
    public void onWriteBlock(View view) {
        ...
        if (!NfcAssistantApplication.isHexAnd16Byte(data, this)) {
            return;
        }
    ...
    private int checkBCC(boolean isWriteBlock) {
        MCReader reader = NfcAssistantApplication.checkForTagAndCreateReader(this);
        if (reader == null) {
        ...
        int uidLen = NfcAssistantApplication.getUID().length;
        if (uidLen != 4) {
        ...
        if (isWriteBlock) {
            bcc = NfcAssistantApplication.hexStringToByteArray(
                    mDataText.getText().toString()
                    .substring(8, 10))[0];
            uid = NfcAssistantApplication.hexStringToByteArray(
                    mDataText.getText().toString()
                    .substring(0, 8));
        } else {
        ...
        boolean isValidBcc;
        try {
            isValidBcc = NfcAssistantApplication.isValidBCC(uid, bcc);
    ...
    private void writeBlock() {
        ...
        byte[][] keys = NfcAssistantApplication.getKeyMap().get(sector);
        int result = -1;

        if (keys[1] != null) {
            result = reader.writeBlock(sector, block,
                    NfcAssistantApplication.hexStringToByteArray(mDataText.getText().toString()),
                    keys[1], true);
        }
    ...
    public void onWriteDump(View view) {
        ...
        if (mWriteDumpFromEditor) {
            ...
            intent.putExtra(FileChooser.EXTRA_DIR,
                    NfcAssistantApplication.getFileFromStorage(NfcAssistantApplication.HOME_DIR + "/" +
                            NfcAssistantApplication.DUMPS_DIR).getAbsolutePath());
    ...
    private void readDumpFromFile(String pathToDump) {
        ...
        String[] dump = NfcAssistantApplication.readFileLineByLine(file, false, this);
    ...
    private void checkDumpAndShowSectorChooserDialog(final String[] dump) {
        int err = NfcAssistantApplication.isValidDump(dump, false);
        if (err != 0) {
            // Error.
            NfcAssistantApplication.isValidDumpErrorToast(err, this);
            return;
        }
        ...
        for (int i = 0; i < dump.length; i++) {
            if (dump[i].startsWith("+")) {
                ...
                mDumpWithPos.get(sector).put(block++,
                        NfcAssistantApplication.hexStringToByteArray(dump[i]));
            } else {
                block++;
            }
        }
     ...
     * @see NfcAssistantApplication#getOperationInfoForBlock(byte, byte,
     * byte, NfcAssistantApplication.Operations, boolean, boolean)
    ...
    private void checkTag() {
        ...
        final SparseArray<byte[][]> keyMap  =
                NfcAssistantApplication.getKeyMap();
        ...
        if (list.size() != 0) {
            ...
            int pad = NfcAssistantApplication.dpToPx(5);
    ...
    private void writeDump(
        ...
        // Create reader.
        final MCReader reader = NfcAssistantApplication.checkForTagAndCreateReader(this);
        if (reader == null) {
            return;
        }
        ...
        LinearLayout ll = new LinearLayout(this);
        int pad = NfcAssistantApplication.dpToPx(10);
        ll.setPadding(pad, pad, pad, pad);
        ll.setGravity(Gravity.CENTER);
        ProgressBar progressBar = new ProgressBar(this);
        progressBar.setIndeterminate(true);
        pad = NfcAssistantApplication.dpToPx(5);
    ...
    private void createFactoryFormattedDump() {
        ...
        // So NfcAssistantApplication.getTag() will return den current present tag
        // (and its size/sector count).
        mDumpWithPos = new HashMap<>();
        int sectors = MifareClassic.get(NfcAssistantApplication.getTag()).getSectorCount();
