...
public class ValueBlockTool extends BasicActivity {
    ...
    public void onDecode(View view) {
        ...
        if (!NfcAssistantApplication.isValueBlock(data)) {
        ...
        // Decode.
        byte[] vbAsBytes = NfcAssistantApplication.hexStringToByteArray(
                data.substring(0, 8));
        // Bytes -> Int. -> reverse.
    ...
    public void onEncode(View view) {
        ...
        String vb = NfcAssistantApplication.byte2HexString(ByteBuffer.allocate(4).putInt(
                Integer.reverseBytes(vbAsInt)).array());
        // Int. -> invert -> reverse -> byte array -> hex string.
        String vbInverted = NfcAssistantApplication.byte2HexString(ByteBuffer.allocate(4).putInt(
                Integer.reverseBytes(~vbAsInt)).array());
    ...
    public void onCopyToClipboard(View view) {
        NfcAssistantApplication.copyToClipboard(mVB.getText().toString(), this, true);
    }
