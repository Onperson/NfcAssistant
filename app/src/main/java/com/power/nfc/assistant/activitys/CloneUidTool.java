...
public class CloneUidTool extends BasicActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (NfcAssistantApplication.getTag() != null) {
            mUid.setText(NfcAssistantApplication.byte2HexString(NfcAssistantApplication.getUID()));
            appendToLog(getString(R.string.text_use_uid_of_scanned_tag)
                + " (" + NfcAssistantApplication.byte2HexString(NfcAssistantApplication.getUID()) + ")");
        }
    }
    ...
    public void onNewIntent(Intent intent) {
        ...
        if (typeCheck != -4) {
            String uid = NfcAssistantApplication.byte2HexString(NfcAssistantApplication.getUID());
            switch (mStatus) {
                case INIT:
    ...
    public void onCalculateBlock0(View view) {
        ...
        byte bcc = NfcAssistantApplication.calcBCC(NfcAssistantApplication.hexStringToByteArray(uid));
    ...
    private void writeManufacturerBlock() {
        ...
        byte[] key = NfcAssistantApplication.hexStringToByteArray(
        ...
        byte[] uid = NfcAssistantApplication.getUID();
        if (uid.length != 4) {
            // Error: no 4 bytes UID. 7 and 10 bytes UID not supported (yet).
            Toast.makeText(this, R.string.info_invalid_uid_length,
                    Toast.LENGTH_LONG).show();
        ...
        int result = reader.writeBlock(0, 0,
                NfcAssistantApplication.hexStringToByteArray(mBlock0Complete), key, keyB);
    ...
    public void onPasteUidFromClipboard(View view) {
        String text = NfcAssistantApplication.getFromClipboard(this);
        if (text != null) {
            mUid.setText(text);
        }
    }
