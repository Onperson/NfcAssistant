...
package com.power.nfc.assistant.abs;
...
import static com.power.nfc.assistant.activitys.Preferences.Preference.AutoCopyUID;
import static com.power.nfc.assistant.activitys.Preferences.Preference.UIDFormat;
import static com.power.nfc.assistant.activitys.Preferences.Preference.UseInternalStorage;

/**
 * NfcAssistantApplication functions and variables for all Activities.
...
public class NfcAssistantApplication extends Application {
    ...
    private static final String LOG_TAG = NfcAssistantApplication.class.getSimpleName();
...
// ############################################################################
    ...
    public static void checkFileExistenceAndSave(final File file,
        ...
        if (file.exists()) {
            ...
            .setPositiveButton(R.string.action_replace,
                    (dialog, which) -> {
                        // Replace.
                        if (NfcAssistantApplication.saveFile(file, lines, false)) {
            ...
            .setNeutralButton(R.string.action_append,
                    (dialog, which) -> {
                        // Append.
                        if (NfcAssistantApplication.saveFileAppend(file, lines, isDump)) {
                            Toast.makeText(context, R.string.info_save_successful,
                                    Toast.LENGTH_LONG).show();
                            activity.onSaveSuccessful();
                        } else {
    ...
    public static boolean hasExternalNfcInstalled(Context context) {
        return NfcAssistantApplication.isAppInstalled("eu.dedb.nfc.service", context);
    }
    ...
    public static boolean isValueBlock(String hexString) {
        byte[] b = NfcAssistantApplication.hexStringToByteArray(hexString);
        if (b.length == 16) {
     ...
     * NfcAssistantApplications.Lang (ArrayUtils) from Apache....
