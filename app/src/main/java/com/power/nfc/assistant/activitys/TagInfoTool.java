...
public class TagInfoTool extends BasicActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        updateTagInfo(NfcAssistantApplication.getTag());
    }

    /**
     * Calls {@link NfcAssistantApplication#treatAsNewTag(Intent, android.content.Context)} and
     * then calls {@link #updateTagInfo(Tag)}
     */
    @Override
    public void onNewIntent(Intent intent) {
        NfcAssistantApplication.treatAsNewTag(intent, this);
    ...
    private void updateTagInfo(Tag tag) {
        ...
        if (tag != null) {
            ...
            mMFCSupport = NfcAssistantApplication.checkMifareClassicSupport(tag, this);

            mLayout.removeAllViews();
            // Display generic info.
            // Create views and add them to the layout.
            TextView headerGenericInfo = new TextView(this);
            headerGenericInfo.setText(NfcAssistantApplication.colorString(
            ...
            int pad = NfcAssistantApplication.dpToPx(5); // 5dp to px.
            ...
            String uid = NfcAssistantApplication.byte2HexString(tag.getId());
            ...
            // Swap ATQA to match the NfcAssistantApplication order like shown here:
            // http://nfc-tools.org/index.php?title=ISO14443A
            byte[] atqaBytes = nfca.getAtqa();
            atqaBytes = new byte[] {atqaBytes[1], atqaBytes[0]};
            String atqa = NfcAssistantApplication.byte2HexString(atqaBytes);
            // SAK in big endian.
            ...
            if (sakBytes[0] != 0) {
                sak = NfcAssistantApplication.byte2HexString(sakBytes);
            } else {
                sak = NfcAssistantApplication.byte2HexString(new byte[] {sakBytes[1]});
            }
            String ats = "-";
            ...
            if (iso != null ) {
                ...
                if (atsBytes != null && atsBytes.length > 0) {
                    ats = NfcAssistantApplication.byte2HexString(atsBytes);
                }
            }
            // Identify tag type.
            ...
            genericInfo.setText(TextUtils.concat(
                    NfcAssistantApplication.colorString(getString(R.string.text_uid) + ":", hc),
                    "\n", uid, "\n",
                    NfcAssistantApplication.colorString(getString(
                            R.string.text_rf_tech) + ":", hc),
                    // Tech is always ISO 14443a due to NFC Intent filter.
                    "\n", getString(R.string.text_rf_tech_14a), "\n",
                    NfcAssistantApplication.colorString(getString(R.string.text_atqa) + ":", hc),
                    "\n", atqa, "\n",
                    NfcAssistantApplication.colorString(getString(R.string.text_sak) + ":", hc),
                    "\n", sak, "\n",
                    NfcAssistantApplication.colorString(getString(
            ...
            if (mMFCSupport == 0) {
                ...
                headerMifareInfo.setText(NfcAssistantApplication.colorString(
